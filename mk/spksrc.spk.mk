...
supported-arch-%: spk_msg
	...
	-@MAKEFLAGS= $(PSTAT_TIME) GCC_DEBUG=$(GCC_DEBUG) $(MAKE) $(addprefix arch-, $*)

publish-supported-arch-%: spk_msg
	@$(MSG) "BUILDING and PUBLISHING package for arch $* (publish-all-supported)" | tee --append build-$*.log
	-@MAKEFLAGS= $(PSTAT_TIME) GCC_DEBUG=$(GCC_DEBUG) $(MAKE) $(addprefix publish-arch-, $*)
...
ifneq ($(filter 1 on ON,$(PSTAT)),)
	...
	@MAKEFLAGS= $(PSTAT_TIME) GCC_DEBUG=$(GCC_DEBUG) $(MAKE) ARCH=$(firstword $(subst -, ,$*)) TCVERSION=$(lastword $(subst -, ,$*)) 2>&1 | tee --append build-$*.log ; \
	  [ $${PIPESTATUS[0]} -eq 0 ] || false
ifneq ($(filter 1 on ON,$(PSTAT)),)
...
ifneq ($(strip $(REQUIRE_KERNEL_MODULE)),)
	...
	@MAKEFLAGS= $(PSTAT_TIME) GCC_DEBUG=$(GCC_DEBUG) $(MAKE) ARCH=$(basename $(subst -,.,$(basename $(subst .,,$*)))) TCVERSION=$(if $(findstring $*,$(basename $(subst -,.,$(basename $(subst .,,$*))))),$(DEFAULT_TC),$(notdir $(subst -,/,$*))) publish 2>&1 | tee --append build-$*.log ; \
	  [ $${PIPESTATUS[0]} -eq 0 ] || false
endif
