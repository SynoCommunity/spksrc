...
# Whether this build uses a gcc overlay rather than the toolchain's stock gcc -- the
# mode (overlay|legacy), known statically here for the status line and diagnostics.
# Overlay is active when the build is not forced legacy and an overlay gcc is
# actually available to select. (TC_GCC_SUFFIX in tc_vars only exists once the work
# dir's tc_vars.mk has been generated, so the dependency walk cannot read it.)
TC_GCC_IS_OVERLAY := $(if $(filter 1 on ON,$(strip $(LEGACY_TOOLCHAIN))),,$(if $(or $(strip $(TC_GCC_VERSION)),$(strip $(TC_OVERLAY_GCC))),1))

# From here on TC_GCC *is* the gcc this build will actually use: the stock one read
# above when legacy, else the pin (TC_GCC_VERSION) or the overlay. Keeping it a single
# variable -- rather than a separate "effective" one -- means a package's feature
# gates (version_ge $(TC_GCC) ...) always judge the compiler that really compiles it,
# with nothing to keep in sync.
TC_GCC := $(if $(strip $(TC_GCC_IS_OVERLAY)),$(or $(strip $(TC_GCC_VERSION)),$(TC_OVERLAY_GCC)),$(TC_GCC))
...
# ---- gcc: judged against the compiler this build will use -------------------
# TC_GCC is the selected compiler (the stock one, or the overlay if one was picked
# above), so a single comparison is right whether or not an overlay is in play: if
# the selected gcc is still too old, the arch is unsupported. Plain ifeq rather than
# a nested $(if): version_ge returns empty for false.
ifneq ($(strip $(MIN_GCC_VERSION)),)
ifneq ($(strip $(TC_GCC)),)
ifeq ($(call version_ge,$(TC_GCC),$(MIN_GCC_VERSION)),)
