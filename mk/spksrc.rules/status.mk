...
#
# TC_GCC (the version) and TC_GCC_IS_OVERLAY (the mode) both come from
...
# (native, toolchain) -- TC_GCC is empty there -- rather than blank.
#
# _status_comma: a literal comma cannot be written inside $(if ...) -- make reads it
# as an argument separator.
_status_comma := ,
STATUS_GCC = $(shell printf '%-23s' "$(if $(strip $(TC_GCC)),GCC: $(strip $(TC_GCC)) ($(if $(strip $(TC_GCC_IS_OVERLAY)),overlay,legacy))$(_status_comma))")
