...
# Package-side default for the rust overlay selector (see rustc.mk). Defined here so
# it is always set before it is passed to the tcvars regen below -- passing an empty
# RUST_OVERLAY= would look like an explicit "off" and error on a tier-3 rust arch.
# A package (or local.mk / CLI) may override it; only custom-rust archs act on it.
RUST_OVERLAY ?= 1
...
$(TCVARS_DONE):
	...
	@# Pass RUST_OVERLAY so the tc_vars generated into THIS package's WORK_DIR reflect
	...
	@$(MAKE) WORK_DIR=$(WORK_DIR) RUST_OVERLAY=$(RUST_OVERLAY) --no-print-directory -C ../../toolchain/$(TC) tcvars
else
$(TCVARS_DONE): ;
endif
