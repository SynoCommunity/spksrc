...
# _tc_get, TC_TARGET, TC_EXTRACT_DIR and the tc-extract target. This component only
# resolves the sysroot for --with-sysroot; the GNU_CONFIGURE build/install flow is the
# framework default (spksrc.build.mk via native-cc.mk), so there are no target overrides here.
###############################################################################
...
TC_SYSROOT_DIR = $(patsubst %/usr/include/,%,$(dir $(firstword $(wildcard \
                   $(TC_EXTRACT_DIR)/$(TC_TARGET)/libc/usr/include/stdio.h \
                   $(TC_EXTRACT_DIR)/$(TC_TARGET)/sys-root/usr/include/stdio.h \
                   $(TC_EXTRACT_DIR)/$(TC_TARGET)/sysroot/usr/include/stdio.h \
                   $(TC_EXTRACT_DIR)/sys-root/usr/include/stdio.h \
                   $(TC_EXTRACT_DIR)/$(TC_TARGET)/usr/include/stdio.h))))
