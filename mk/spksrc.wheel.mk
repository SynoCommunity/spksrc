...
build_wheel_target: $(PRE_WHEEL_TARGET)
	@if [ -n "$(WHEELS)" ] ; then \
		...
		if [ -s "$(WHEELHOUSE)/$(WHEELS_CROSSENV_COMPILE)" ]; then \
			...
			if [ -z "$(CROSSENV)" ]; then \
				$(RUN) BUILD_ARGS=$(WHEELS_BUILD_ARGS) _PYTHON_HOST_PLATFORM="$(TC_TARGET)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" CPPFLAGS="$(CPPFLAGS) $(WHEELS_CPPFLAGS)" CXXFLAGS="$(CXXFLAGS) $(WHEELS_CXXFLAGS)" $(PIP) $(PIP_WHEEL_ARGS) --requirement $(WHEELHOUSE)/$(WHEELS_CROSSENV_COMPILE) ; \
			else \
				. $(CROSSENV) && $(RUN) BUILD_ARGS=$(WHEELS_BUILD_ARGS) _PYTHON_HOST_PLATFORM="$(TC_TARGET)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" CPPFLAGS="$(CPPFLAGS) $(WHEELS_CPPFLAGS)" CXXFLAGS="$(CXXFLAGS) $(WHEELS_CXXFLAGS)" $(PIP_CROSS) $(PIP_WHEEL_ARGS) --no-build-isolation --requirement $(WHEELHOUSE)/$(WHEELS_CROSSENV_COMPILE) ; \
			fi ; \
		fi ; \
		if [ -s "$(WHEELHOUSE)/$(WHEELS_LIMITED_API)" ]; then \
			$(MSG) "Force limited API $(PYTHON_LIMITED_API)" ; \
			. $(CROSSENV) && $(RUN) BUILD_ARGS=$(WHEELS_BUILD_ARGS) _PYTHON_HOST_PLATFORM="$(TC_TARGET)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" CPPFLAGS="$(CPPFLAGS) $(WHEELS_CPPFLAGS)" CXXFLAGS="$(CXXFLAGS) $(WHEELS_CXXFLAGS)" $(PIP_CROSS) $(PIP_WHEEL_ARGS) --build-option='--py-limited-api=$(PYTHON_LIMITED_API)' --no-build-isolation --requirement $(WHEELHOUSE)/$(WHEELS_LIMITED_API) ; \
		fi ; \
	fi
