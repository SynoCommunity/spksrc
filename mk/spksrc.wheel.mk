...
build_wheel_target: $(PRE_WHEEL_TARGET)
	@if [ ! -z "$(WHEELS)" ] ; then \
		...
		if [ ! -z "$(CROSS_COMPILE_WHEELS)" ] ; then \
			...
			if [ -z "$(CROSSENV)" ]; then \
				$(RUN) _PYTHON_HOST_PLATFORM="$(TC_PREFIX)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" $(PIP_WHEEL) ; \
			else \
				. $(CROSSENV) && $(RUN) _PYTHON_HOST_PLATFORM="$(TC_PREFIX)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" pip $(PIP_WHEEL_ARGS) --no-build-isolation ; \
			fi ; \
		else \
