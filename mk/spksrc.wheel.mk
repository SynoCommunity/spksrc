	...
	@if [ -n "$(WHEELS)" ] ; then \
		...
		if [ -s "$(WHEELHOUSE)/$(WHEELS_CROSSENV_COMPILE)" ]; then \
			...
			if [ -z "$(CROSSENV)" ]; then \
				...
				. $(CROSSENV) && $(RUN) _PYTHON_HOST_PLATFORM="$(TC_TARGET)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" CPPFLAGS="$(CPPFLAGS) $(WHEELS_CPPFLAGS)" CXXFLAGS="$(CXXFLAGS) $(WHEELS_CXXFLAGS)" $(PIP_CROSS) $(PIP_WHEEL_ARGS) $(addprefix --global-option=,$(WHEELS_BUILD_ARGS)) --no-build-isolation --requirement $(WHEELHOUSE)/$(WHEELS_CROSSENV_COMPILE) ; \
			fi ; \
		fi ; \
		if [ -s "$(WHEELHOUSE)/$(WHEELS_LIMITED_API)" ]; then \
			$(MSG) "Force limited API $(PYTHON_LIMITED_API)" ; \
			. $(CROSSENV) && $(RUN) _PYTHON_HOST_PLATFORM="$(TC_TARGET)" CFLAGS="$(CFLAGS) -I$(STAGING_INSTALL_PREFIX)/$(PYTHON_INC_DIR) $(WHEELS_CFLAGS)" LDFLAGS="$(LDFLAGS) $(WHEELS_LDFLAGS)" CPPFLAGS="$(CPPFLAGS) $(WHEELS_CPPFLAGS)" CXXFLAGS="$(CXXFLAGS) $(WHEELS_CXXFLAGS)" $(PIP_CROSS) $(PIP_WHEEL_ARGS) $(addsuffix --global-option=,$(WHEELS_BUILD_ARGS))) --build-option='--py-limited-api=$(PYTHON_LIMITED_API)' --no-build-isolation --requirement $(WHEELHOUSE)/$(WHEELS_LIMITED_API) ; \
		fi ; \
	fi
