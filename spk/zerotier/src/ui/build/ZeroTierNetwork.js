	...
	leaveNetwork: function (event) {
		Ajax.call({
			...
			success: function (data) {
				if (this.props.onNetworkDeleted) this.props.onNetworkDeleted(this.props.nwid);
			}.bind(this),
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						'div',
						{ className: 'value zeroTierAddress' },
						this.props['mac']
					)
				),
				React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						'div',
						{ className: 'value ipList' },
						this.props['assignedAddresses'].map(function (ipAssignment) {
							return React.createElement(
								'div',
								{ key: ipAssignment, className: 'ipAddress' },
								ipAssignment
							);
						})
					)
			...
			React.createElement(
				'button',
				{ type: 'button', className: 'leaveNetworkButton', onClick: this.leaveNetwork },
				'Leave\xA0Network'
			)
		);
	}
