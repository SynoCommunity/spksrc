var ZeroTierNode = React.createClass({
	...
	updatePeers: function () {
		Ajax.call({
			url: window.ui_addr + 'peer' + '?' + window.COOKIE_KEY + '=' + window.COOKIE_VAL + window.CSRF_TOKEN_KEY + '=' + window.CSRF_TOKEN_VAL,
			type: 'GET',
	...
	updateNetworks: function () {
		Ajax.call({
			url: window.ui_addr + 'network' + '?' + window.COOKIE_KEY + '=' + window.COOKIE_VAL + window.CSRF_TOKEN_KEY + '=' + window.CSRF_TOKEN_VAL,
			type: 'GET',
			success: function (data) {
				if (data) {
	...
	dispatchRequest: function (path, parameters) {
		Ajax.call({
			url: window.ui_addr + path + '?' + window.COOKIE_KEY + '=' + window.COOKIE_VAL + window.CSRF_TOKEN_KEY + '=' + window.CSRF_TOKEN_VAL,
			type: 'GET',
	...
	joinNetwork: function (event) {
		...
		if (this.networkToJoin && this.networkToJoin.length === 16) {
			Ajax.call({
				url: window.ui_addr + 'network/' + this.networkToJoin + '?' + window.COOKIE_KEY + '=' + window.COOKIE_VAL + window.CSRF_TOKEN_KEY + '=' + window.CSRF_TOKEN_VAL,
				type: 'POST',
				success: function (data) {
					this.networkToJoin = '';
					if (this.networkInputElement) this.networkInputElement.value = '';
					this.updateNetworks();
				}.bind(this),
				error: function () {}.bind(this)
			});
	...
	resetService: function (event) {
		event.preventDefault();
		Ajax.call({
			url: window.ui_addr + 'reset' + '?' + window.COOKIE_KEY + '=' + window.COOKIE_VAL + window.CSRF_TOKEN_KEY + '=' + window.CSRF_TOKEN_VAL,
			type: 'POST',
			success: function (data) {
				// ...
			}.bind(this),
	...
	handleNetworkIdEntry: function (event) {
		...
		if (nid) {
			...
			for (var i = 0; i < nid.length && i < 16; ++i) {
				if ("0123456789abcdef".indexOf(nid.charAt(i)) >= 0) nnid += nid.charAt(i);
			}
	...
	handleNetworkDelete: function (nwid) {
		...
		for (var i = 0; i < this.state._networks.length; ++i) {
			if (this.state._networks[i].nwid !== nwid) networks.push(this.state._networks[i]);
		}
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						"div",
						{ className: "middleScroll" },
						React.createElement(
							"div",
							{ className: "networks", key: "_networks" },
			...
			React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						"form",
						{ onSubmit: this.joinNetwork },
						React.createElement("input", { type: "text", maxlength: "16", placeholder: "[ Network ID ]", onChange: this.handleNetworkIdEntry, size: "16" }),
						React.createElement(
							"button",
							{ type: "button", onClick: this.joinNetwork },
							"Join"
						)
					)
				)
