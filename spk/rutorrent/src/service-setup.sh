...
SERVICE_COMMAND="env RTORRENT_RC=${RTORRENT_RC} HOME=${SYNOPKG_PKGVAR} PATH=${PATH} LD_LIBRARY_PATH=${SYNOPKG_PKGDEST}/lib:${LD_LIBRARY_PATH} ${SERVICE_COMMAND}"
...
service_postinst ()
{
    ...
    # Configure files
    if [ "${SYNOPKG_PKG_STATUS}" == "INSTALL" ]; then
        TOP_DIR=$(echo "${wizard_download_volume:=/volume1}/${wizard_download_dir:=downloads}" | cut -d "/" -f 2)
