...
def default_config():
    ...
    session.add(Backend(id=1, name=u'web', servers=u'web localhost:80 check', options='option httpchk'))
    session.add(Backend(id=2, name=u'dsm', servers=u'dsm localhost:5000 check', options='option httpchk /webman/index.cgi'))
    session.add(Backend(id=3, name=u'sabnzbd', servers=u'sabnzbd localhost:8080 check', options='option httpchk, http-check expect status 401'))
    session.add(Backend(id=4, name=u'nzbget', servers=u'nzbget localhost:6789 check', options='option httpchk'))
    session.add(Backend(id=5, name=u'sickbeard', servers=u'sickbeard localhost:8081 check', options='option httpchk, http-check expect status 401'))
    session.add(Backend(id=6, name=u'couchpotatoserver', servers=u'couchpotatoserver localhost:5050 check', options='option httpchk'))
    session.add(Backend(id=7, name=u'headphones', servers=u'headphones localhost:8181 check', options='option httpchk, http-check expect status 401'))
    session.add(Backend(id=8, name=u'maraschino', servers=u'maraschino localhost:8260 check', options='option httpchk'))
    ...
    session.add(Backend(id=11, name=u'gateone', servers=u'gateone localhost:8271 ssl check', options='option ssl-hello-chk'))
