...
service_postinst ()
{
    ...
    if [ -e "${SYNOPKG_PKGDEST}/lib/$LIBRARY_WITH_VERSION" ]; then
        ...
        ln -s "${SYNOPKG_PKGDEST}/lib/$LIBRARY" "/usr/local/lib/$LIBRARY_WITH_VERSION"
        ln -s "${SYNOPKG_PKGDEST}/lib/${LIBRARY}.${LIBRARY_MAIN_VERSION}" "/usr/local/lib/$LIBRARY_WITH_VERSION"
    fi
}
