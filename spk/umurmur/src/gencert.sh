...
${OPENSSL} req -x509 -newkey rsa:1024 -keyout ${INSTALL_DIR}/etc/umurmur.key -nodes -sha1 -days 365 -out ${INSTALL_DIR}/etc/umurmur.crt -config ${INSTALL_DIR}/var/openssl.cnf < ${INSTALL_DIR}/var/cert.fields > /dev/null 2>&1
