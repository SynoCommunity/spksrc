...
# SYNOPKG_PKGDEST 等于 /volume1/@appstore/ZdinNav
# SYNOPKG_PKGVAR  等于 /volume1/@appdata/ZdinNav
# PID_FILE  等于 /volume1/@appdata/ZdinNav/ZdinNav.pid
# LOG_FILE 等于 /volume1/@appdata/ZdinNav/ZdinNav.log
...
validate_preinst() {
	...
	# # 子级文件夹设置755
	# find "${zdinnav_data_folder}" -type d -exec chmod 755 "{}" \;
	# # 子级文件设置755
	# chmod -R 755 "${zdinnav_data_folder}"* 2>/dev/null || true
	# 创建目录（容错，防止文件路径不存在）
...
service_postinst() {
	...
	# # 子级文件夹设置755
	# find "${SYNOPKG_PKGDEST}/application/" -type d -exec chmod 755 "{}" \;
	# # 子级文件夹设置755
	# find "${zdinnav_data_folder}" -type d -exec chmod 755 "{}" \;
	# # 子级文件设置755
	# chmod -R 755 "${zdinnav_data_folder}"* 2>/dev/null || true
