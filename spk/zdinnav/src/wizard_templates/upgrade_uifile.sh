...
# 与install_uifile.sh安装向导基本一直(多了部分配置)
get_wizard_zdinnav_upgrade() {
	# 默认初始化配置
	...
	if [ -f "$ZDINNAV_DATA_RECORDING_PATH" ]; then
		...
		# 单独放在wizard_config.json这里方便用户可以删除，重置
		local zdinnav_wizard_json="${zdinnav_folder_path}configuration/wizard_config.json"
		if [ -f "$zdinnav_wizard_json" ]; then
			# 子级文件夹设置755
			@find "$(zdinnav_folder_path)" -type d -exec chmod 755 "{}" \;
			# 子级文件设置755
			@chmod -R 755 "$(zdinnav_folder_path)/*" 2>/dev/null || true
			# 读取配置文件信息
	...
	local wizard_upgrade=$(
		/bin/cat <<EOF
  {
    "step_title": "{{{ZDINNAV_WIZARD_UPGRADE_TITLE}}}",
    "items": [{
        "desc": "{{{ZDINNAV_WIZARD_UPGRADE_DESCRIPTION}}}"
	}]
  },
  {
    "step_title": "{{{ZDINNAV_WIZARD_UPGRADE_TITLE}}}",
...
main() {
	local upgrade_page=""
	local wizard_upgrade_config=$(get_wizard_zdinnav_upgrade)
	upgrade_page=$(page_append "$upgrade_page" "$wizard_upgrade_config")
