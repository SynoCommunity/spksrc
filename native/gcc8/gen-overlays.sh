...
#   GCC8_RELEASE=<release-tag> ./gen-overlays.sh 6.2.4 7.0
set -e
cd "$(dirname "$0")/../../toolchain"
RELEASE="${GCC8_RELEASE:-toolchains%2Fdsm6.2.4}"
VERSIONS="${@:-6.2.4 7.0}"
n=0
...
TC_GLIBC  := \$(shell grep -E '^TC_GLIBC' ../\$(TC_BASE)/Makefile 2>/dev/null | sed 's/.*= *//')

PKG_NAME = gcc8-\$(TC_ARCH)
PKG_VERS = 8.5.0
PKG_EXT  = txz
# glibc-tagged: some archs share a triple across DSM versions but differ in glibc
# (e.g. the comcerto2k orphan stays on 2.20 while its siblings move to 2.26).
PKG_DIST_NAME = gcc8-\$(TC_TARGET)-glibc\$(TC_GLIBC)-\$(TC_VERS).\$(PKG_EXT)
